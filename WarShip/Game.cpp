...
void Game::placeShips() {
	...
	while (player.manager.endOfSetting()) {  // добавить !   не забыть
		int length, x, y;
		char o;
		...
		Orientation orient = o == 'v' ? VERTICAL : HORIZONTAL;
			// throw std::invalid_argument();    ; обработчик ошибок There are only two genders!
	...
	for (int length = 1; length <= 4; length++) {
		for (int j = 0; j < number_of_ships[length]; j++) {
			do {
				random_position_x = rand() % player.field.length;
				random_position_y = rand() % player.field.width;
...
void Game::printInfoAbilities() {
	std::cout << "There are 3 abilities in the game:\n\t0 - DoubleDamage. Your next attack will deal double damage\n";
	std::cout << "\t1 - Scanner. Requires 2 arguments - x, y - coordinates. Check if there is a ship or not in the 2x2 square area.\n";
	std::cout << "\t2 - Shelling. Hit random ship segment.\n";
}
...
void Game::printMyField(InterfaceSymbols& registers) {
	player.field.print(registers);
}

void Game::printOppsField(InterfaceSymbols& registers) {
	opp.field.print(registers);
}
